function [totalCosts,shortestPaths] = kShortestPath(netCostMatrix, source, destination, k_paths)
% clear
% clc
% mpc = case39;
% [L,~] = size(mpc.branch);
...
if source > size(netCostMatrix,1) || destination > size(netCostMatrix,1)
    ...
    %---------------------INITIALIZATION---------------------
    k=1;
    [cost,path] = mydijkstra(netCostMatrix, source, destination);
    ...
    if isempty(path)
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                ...
                [c,dev_p] = mydijkstra(temp_netCostMatrix, P_(index_dev_vertex), destination);
                if ~isempty(dev_p)
